...
int main(){
	...
	cv::rectangle(image1,center - cv::Point(radius,radius),center+cv::Point(radius,radius), 255,3);
	
	cv::Mat image2;
	cv:cvtColor(image1,image2,CV_GRAY2BGR);
	int inscibed_radius = radius / sqrt(2);
	cv::Rect rect( center - cv::Point(inscibed_radius,inscibed_radius), center + cv::Point(inscibed_radius,inscibed_radius));
	
	cv::Mat roi = image2(rect);
	roi.setTo(cv::Scalar(0,185,119));
